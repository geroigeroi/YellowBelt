...
public:
    TeamTasks(std::string Person) {
        TasksInfo forConst;
        forConst[TaskStatus::NEW] = 0;
        forConst[TaskStatus::IN_PROGRESS] = 0;
        forConst[TaskStatus::TESTING] = 0;
        forConst[TaskStatus::DONE] = 0;
...
int main() {
    ...
    TeamTasks Alice("Alice");
    for (int i = 0; i < 3; ++i) {
        Alice.AddNewTask("Alice");
    }
    Alice.PerformPersonTasks("Alice", 5);
    Alice.PerformPersonTasks("Alice", 5);
    Alice.PerformPersonTasks("Alice", 1);
    for (int i = 0; i < 5; ++i) {
        Alice.AddNewTask("Alice");
    }
    Alice.PerformPersonTasks("Alice", 2);
    Alice.GetPersonTasksInfo("Alice");
